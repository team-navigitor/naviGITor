...
socket.emit("subscribe", { room: 'team-navigitor.navigitorlive' });
...
socket.on('incomingCommit', function(data){
	console.log('im outside of react component');
	socket.emit("subscribe", { room: 'team-navigitor.navigitorlive' });
});
...
export default class GitTree extends Component {
	...
	componentDidMount() {
		socket.on('incomingCommit', function(data){
			...
			console.log(localGitNodes);
			console.log('im inside react component');
		}.bind(this));
	}
